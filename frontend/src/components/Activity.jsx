...
function Activity() {
	...
	return (
		<div className="w-100 md:w-170 mx-auto">
			...
			<motion.p animate={{ opacity: 1, transition: { opacity: { duration: 1, ease: "easeInOut" } } }} initial={{ opacity: 0 }} className="text-xl text-center mt-2">right click a post for options.</motion.p> {/* Adding layout prop to prevent scaling issues. */}
			{
				render && <AnimatePresence>
					{
						posts.map((item) => {
							return <div key={item.id} onContextMenu={(e) => onContextMenu(e, item)}>
								<Post postId={item.id} userLiked={item.userLiked} communityName={item.communityName} communityImage={item.communityImage} date={item.date} title={item.title} image={item.image} text={item.text} likes={item.likes} />
							</div>
						})
					}
					{
